...
These are photographs published on the original school website. They are historical material, not evidence of current classes, staff or availability. Dates, photographer credits, rights, consents and production attribution remain **NEEDS VERIFICATION**. Do not identify pictured people.
...
The hero uses one placeholder placement. It is an existing stock photograph, **not a photograph of Crazy Gang School, its students, teachers or lessons**, with empty decorative alt text and no visible caption. It is marked in code only by `placeholder: true` in `src/content.js` (which sets a non-visible `data-placeholder` attribute). Both prepared source sets remain independently replaceable for later approved photography.
...
The photographic placeholders illustrate composition only. Juxtaposition with course names does not identify a lesson, age group or discipline.
...
The seven course images are visual placeholders downloaded on 6 September 2026. They are **not photographs of Crazy Gang School, its students, teachers or lessons** and do not document the current courses. In the interface they carry no visible “placeholder” label anymore; each `<img>` keeps a non-visible `data-placeholder` attribute and the source page credit is shown as the photo caption on the course page. Replace each set independently when original, approved school photography is available.
...
These are source-site materials. Current staff composition, image rights, photographer credits and consent for reuse remain **NEEDS VERIFICATION** before launch (see `LAUNCH_CHECKLIST.md`).
...
The public listing showed a 4.8 rating on 6 September 2026. The reviews section shows that rating plus five review texts transcribed verbatim from the public listing on 9 September 2026 (owner-supplied screenshots), with author name and star count; dates are omitted. See `src/ReviewsSection.jsx` (`verifiedReviews`). Any further review is added only after direct verification against the listing, and any entry is removed on request per the Privacy Policy.

No remote image, font, video or tracking embed is loaded by the homepage or course pages. The Google Maps iframe is the only remote embed and is not mounted until the visitor activates it. Visitor statistics are planned to use cookieless Cloudflare Web Analytics (dashboard, no code) and are not enabled yet. Confirm placeholder licenses, archival image rights/credits and consents, logo approval, current operational content and final alt/captions before public launch — see `LAUNCH_CHECKLIST.md`.
