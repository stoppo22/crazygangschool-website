...
No remote image, font, video or tracking embed is loaded by the homepage or course pages. The Google Maps iframe is the only remote embed and is not mounted until the visitor activates it. Visitor statistics, once enabled, use cookieless Cloudflare Web Analytics (dashboard, no code). Confirm placeholder licenses, archival image rights/credits and consents, logo approval, current operational content and final alt/captions before public launch — see `LAUNCH_CHECKLIST.md`.
