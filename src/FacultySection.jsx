...
function ArtisticDirection() {
  ...
  return <div className="direction" aria-labelledby="direction-title">
    <h3 id="direction-title">La direzione artistica</h3>
    <div>{directors.map(person => <article key={person.id}><strong>{person.name}</strong><span>Direttore artistico</span></article>)}</div>
  </div>;
}
...
export function FacultySection() {
  ...
  const items = useMemo(() => faculty.map(teacher => ({ value: teacher.id, title: <span className="faculty-row"><span className="faculty-row__name">{teacher.name}</span><span className="faculty-row__line" aria-hidden="true" /></span>, content: <TeacherProfile teacher={teacher} mobile /> })), []);
