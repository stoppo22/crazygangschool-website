...
function Wordmark({ footer = false, onNavigate }) {
  return <a href="#inizio" onClick={onNavigate} className={`wordmark ${footer ? 'wordmark-footer' : ''}`} aria-label="Crazy Gang School, torna all'inizio"><span>CRAZY<br />GANG<span className="brand-dot">.</span></span><span className="brand-school">DANCE<br />SCHOOL</span></a>;
}

function Photo({ name, className = '', priority = false, caption = true }) {
  const photo = photos[name];
  return <figure className={`photo ${className}`}><img src={photo.src} alt="" width={name === 'studio' ? 1100 : 1366} height={name === 'studio' ? 1650 : 1820} style={{ objectPosition: photo.position }} loading={priority ? 'eager' : 'lazy'} fetchPriority={priority ? 'high' : 'auto'} />{caption && <figcaption>Fotografia segnaposto</figcaption>}</figure>;
}
...
function Disciplines() {
  ...
  return <section id="discipline" tabIndex={-1} className="disciplines section-pad">
    <div className="section-intro"><div><span className="eyebrow">Le discipline</span><h2>Un corpo.<br /><span className="serif">Tanti linguaggi.</span></h2></div><p>Classica, moderna, hip hop e non solo.<br />Esplora le discipline raccontate dalla scuola e chiedi informazioni sul percorso che ti interessa.</p></div>
    <div className="course-grid" style={{ '--course-columns': courseGroups.map((_, i) => `minmax(0, ${active === i ? 6 : 3}fr)`).join(' ') }}>{courseGroups.map((group, index) => <article key={group.id} className={`course-panel ${active === index ? 'active' : ''}`}>
      <Photo name={group.photo} caption={false} />
      <div className="course-shade" />
      <button className="course-trigger" id={`trigger-${group.id}`} aria-expanded={active === index} aria-controls={`panel-${group.id}`} onClick={() => setActive(index)} onMouseEnter={() => { if (matchMedia('(hover: hover)').matches) setActive(index); }}><span>{group.title}</span><span className="course-symbol" aria-hidden="true">{active === index ? '−' : '+'}</span></button>
    ...
    <div className="course-note"><p>Per corsi attivi, livelli e orari, contatta direttamente la scuola.</p><span>Fotografie segnaposto</span></div>
...
function App() {
  ...
  useGSAP(() => {
    ...
    media.add('(prefers-reduced-motion: no-preference)', () => {
      ...
      gsap.fromTo('.story-word', { opacity: 0.18 }, { opacity: 1, stagger: 0.08, ease: 'none', scrollTrigger: { trigger: '.story-copy', start: 'top 88%', end: 'bottom 52%', scrub: 0.6 } });
  ...
  return <div ref={root}>
    ...
    <Navigation />
    <main id="contenuto" className="page-shell">
      <section id="inizio" className="hero" aria-labelledby="hero-title">
        ...
        <h1 className="hero-title" id="hero-title"><span>LA DANZA,</span><span>IN OGNI <em>FORMA.</em></span></h1>
        <div className="hero-bottom"><div className="hero-copy"><p>Crazy Gang School.<br />Dalla scoperta della danza<br />all’esperienza del palcoscenico.</p><a className="button button-orange" href="#discipline">Esplora le discipline <Arrow diagonal /></a></div><a className="hero-scroll" href="#scuola"><span>Entra nel nostro mondo</span><Arrow /></a></div>
        <span className="hero-side" aria-hidden="true">CORPO. SPAZIO. ESPRESSIONE.</span>
      </section>
      ...
      <section id="scuola" tabIndex={-1} className="school section-pad">
        ...
        <h2 className="story-copy" aria-label={story}>{story.split(' ').map((word, i) => <React.Fragment key={i}>{i === 8 && <span className="inline-photo" aria-hidden="true"><img src={photos.studio.src} alt="" loading="lazy" /></span>}<span className="story-word" aria-hidden="true">{word} </span></React.Fragment>)}</h2>
